...
3. Parado en esta carpeta (`uadel-dashboard/`), corré:
   ...
   confirmar. No hace falta ningún archivo de configuración: Vercel sirve
   `index.html` (en la raíz del proyecto) como sitio estático, y todo lo
   que está en `api/` como función automáticamente. Al terminar te da una
   URL de prueba tipo `uadel-dashboard.vercel.app` — todavía no va a
   funcionar porque faltan las variables de entorno (paso 3).
